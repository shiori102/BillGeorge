...
//OPTIONAL STEP 5
scraper.listingPages.push('https://www.billgeorge.org/tv-and-video/');

//STEP 6 - SCRAPE EXCERPTS
scraper.scrapeListingPages('.tv-and-video-link', (item) => { 
    var video = {
      'video_title':$(item).find('h3.tv-and-video-title').first().text().trim(),
      'video_date':$(item).find('p.tv-and-video-date').first().text().trim(),
      'video_link':$(item).attr('href'),
      'video_thumb':$(item).find('img.tv-and-video-image').attr('src'),
      'video_alt':$(item).find('img.tv-and-video-image').attr('alt')};
      
      var type = $(item).siblings('h2').first().text().trim();
...
//STEP 7 - SCRAPE INDIVIDUAL POSTS
      ...
      //   item.post_content = [];
      //   var article = $doc.find('article').first();
